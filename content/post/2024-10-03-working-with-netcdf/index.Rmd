...
## Climatic Data in Hydrologcal analysis
Climatic data such as temperature and precipation is essenstial for any hydrological analysis. Most of time several year of data is required to do any type of hydrological analysis. The climatic data is obtained from either the settlite based observation or insitu observation.

For hydrological analysis historical precipation and temperature data is essenstial. Usually these data can be obtained from Indian Meterolgocal Department (भारतीय मौसम विज्ञान विभाग:IMD) free of cost.

IMD pune provide the gridded precipitation and Temperature data since 1901. Most of time I find students are struggling to process the data that this either in NetCDF (ext: ".nc"), or Binary data (ext: ".grd"). The processign of both the data is a little different.

## Data source, specifications, and metadata.
To proceess any data the most important step is metadata. A metdata for temperature and precipation shown:


### Rainfall Data:
Rainfall data product is resolution daily gridded rainfall data (0.25 x 0.25 degree). The unit of rainfall is in millimeter (mm). Data available for 122 years, 1901 to 2023. Data is arranged in 135x129 grid points. The first data in the record is at 6.5N & 66.5E, the second is at 6.5N & 66.75E, and so on. The last data record corresponds to 38.5N & 100.0E. The yearly data file consists of 365/366 records corresponding to non leap/ leap years. You can download it from <https://imdpune.gov.in/cmpg/Griddata/Rainfall_25_NetCDF.html>

### Temperature data (min/max)
IMD High resolution $1\times1$ degree gridded daily temperature data (1951-2018)*. This data is arranged in 31x31 grid points. Lat 7.5N, 8.5N ... 36.5, 37.5 (31 Values). Long 67.5E, 68.5E ... 96.5, 97.5 (31 Values). For leap years, data for 366 days are included. The unit of tempereture is in Celcious.
...
As an example I am showing the metadata of IMD precipation file.
...
Based on the aformensioned NetCDF metadata, it consist of three main components:
...
NetCDF file have 3 dimension `lat`, `lon`, and `time (day)`
...
## Extracting the daily data with `extract`
